...
class SetupController(QObject):
    ...
    def _connect_setup_widgets(self) -> None:
        ...
        self._widget.txt_target.textChanged.connect(self._txt_target_text_changed)
        self._widget.txt_prefix.textChanged.connect(self._txt_prefix_text_changed)
        self._widget.txt_filename.textChanged.connect(
            self._txt_filename_text_changed
        )
    ...
    def _update_setup_values(self) -> None:
        ...
        self._widget.txt_target.setText(self.model.target)
        self._widget.txt_prefix.setText(self.model.prefix)
        self._widget.txt_filename.setText(self.model.filename)

    def _update_basedir(self) -> None:
        """Updates the current base directory and updates the GUI path label."""

        target = ""
        if not self.model.target.strip() == "":
            target = self.model.target + "/"

        prefix = self.model.prefix
        # prefix = ""
        # if not self.model.prefix.strip() == "":
        #     prefix = self.model.prefix + "/"

        filename = self.model.filename
        # filename = ""
        # if not self.model.filename.strip() == "":
        #     filename = self.model.filename + "/"

        self.basedir = f"C:/Data/Batteries/{target}{prefix}{filename}"
    ...
    def _txt_target_text_changed(self) -> None:
        """Updates the current hutch value based on user input."""
        self.model.target = self._widget.txt_target.text()
        self._update_basedir()

    def _txt_prefix_text_changed(self) -> None:
        """Updates the current cycle value based on user input."""
        self.model.prefix = self._widget.txt_prefix.text()
        self._update_basedir()

    def _txt_filename_text_changed(self) -> None:
        """Updates the current filename value based on user input."""
        self.model.filename = self._widget.txt_filename.text()
        self._update_basedir()
